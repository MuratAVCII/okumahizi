...
class FivePoint extends StatefulWidget {
  const FivePoint({Key? key}) : super(key: key);

  @override
  _FivePointState createState() => _FivePointState();
}

class _FivePointState extends State<FivePoint> {
      ...
      if (!_paused) {
        setState(() {
          if (isPositiveRotation) {
            _rotation += 2 * math.pi / 5; // 90 derece dönüş
          } else {
            _rotation -= 2 * math.pi / 5; // -90 derece dönüş
          }
                  ...
                  children: [
                    ...
                    TextButton(
                      onPressed: () {
                        Navigator.pushReplacement(
                          context,
                          MaterialPageRoute(builder: (context) => FivePoint()),
                        ); // Restart the game
                      },
...
class PentagonPainter extends CustomPainter {
  ...
  PentagonPainter(this.rotation, this.screenWidth, this.screenHeight);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      if (i == 0) {
        pentagonPath.moveTo(center.dx + pentagonSize * math.cos(angle) / 2,
            center.dy - pentagonSize * math.sin(angle) / 2);
      } else {
        pentagonPath.lineTo(center.dx + pentagonSize * math.cos(angle) / 2,
            center.dy - pentagonSize * math.sin(angle) / 2);
      }
    }
    pentagonPath.close();

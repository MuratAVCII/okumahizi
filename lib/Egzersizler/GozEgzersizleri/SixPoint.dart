...
class SixPoint extends StatefulWidget {
  const SixPoint({Key? key}) : super(key: key);

  @override
  _SixPointState createState() => _SixPointState();
}

class _SixPointState extends State<SixPoint> {
      ...
      if (!_paused) {
        setState(() {
          if (isPositiveRotation) {
            _rotation += 2 * math.pi / 6; // 60 derece dönüş
          } else {
            _rotation -= 2 * math.pi / 6; // -60 derece dönüş
          }
                  ...
                  children: [
                    ...
                    TextButton(
                      onPressed: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(builder: (context) => SixPoint()),
                        ); // Restart the game
                      },
...
class HexagonPainter extends CustomPainter {
  ...
  HexagonPainter(this.rotation, this.screenWidth, this.screenHeight);

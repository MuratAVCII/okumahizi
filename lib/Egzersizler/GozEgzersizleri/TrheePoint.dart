...
class ThreePoint extends StatefulWidget {
  const ThreePoint({Key? key}) : super(key: key);

  @override
  _ThreePointState createState() => _ThreePointState();
}

class _ThreePointState extends State<ThreePoint> {
      ...
      if (!_paused) {
        setState(() {
          if (isPositiveRotation) {
            _rotation += 2 * math.pi / 3; // 90 derece dönüş
          } else {
            _rotation -= 2 * math.pi / 3; // -90 derece dönüş
          }
                  ...
                  children: [
                    TextButton(
                      onPressed: () {
                        ...
                        ); // Restart the game // Go back to the previous page
                      },
                      child: Text('Geri Dön'),
                    ),
                    TextButton(
                      onPressed: () {
                        Navigator.pushReplacement(
                          context,
                          MaterialPageRoute(builder: (context) => ThreePoint()),
                        ); // Restart the game
                      },
...
class TrianglePainter extends CustomPainter {
  ...
  TrianglePainter(this.rotation, this.screenWidth, this.screenHeight);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    double triangleSize = size.width;
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if (i == 0) {
        trianglePath.moveTo(
          center.dx + triangleSize * math.cos(angle) / 2,
          center.dy - triangleSize * math.sin(angle) / 2,
        );
      } else {
        trianglePath.lineTo(
          center.dx + triangleSize * math.cos(angle) / 2,
    ...
    final Offset redDotPosition = Offset(
      center.dx + triangleSize * math.cos(math.pi / 2) / 2,
      center.dy - triangleSize * math.sin(math.pi / 2) / 2,
    );

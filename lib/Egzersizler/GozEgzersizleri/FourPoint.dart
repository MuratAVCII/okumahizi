...
class FourPoint extends StatefulWidget {
  const FourPoint({Key? key}) : super(key: key);

  @override
  _FourPointState createState() => _FourPointState();
}

class _FourPointState extends State<FourPoint> {
      ...
      if (!_paused) {
        setState(() {
          if (isPositiveRotation) {
            _rotation += 2 * math.pi / 4; // 90 derece dönüş
          } else {
            _rotation -= 2 * math.pi / 4; // -90 derece dönüş
          }
                  ...
                  children: [
                    ...
                    TextButton(
                      onPressed: () {
                        Navigator.pushReplacement(
                          context,
                          MaterialPageRoute(builder: (context) => FourPoint()),
                        ); // Restart the game
                      },
...
class SquarePainter extends CustomPainter {
  ...
  SquarePainter(this.rotation, this.screenWidth, this.screenHeight);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Kare köşelerine kırmızı noktayı ekle
    final topLeftCorner = Offset(square.left, square.top);
    canvas.drawCircle(topLeftCorner, dotRadius, redDotPaint);
  }

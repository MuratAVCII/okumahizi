...
class SevenPoint extends StatefulWidget {
  const SevenPoint({Key? key}) : super(key: key);

  @override
  _SevenPointState createState() => _SevenPointState();
}

class _SevenPointState extends State<SevenPoint> {
      ...
      if (!_paused) {
        setState(() {
          if (isPositiveRotation) {
            _rotation += 2 * math.pi / 7; // 60 derece dönüş
          } else {
            _rotation -= 2 * math.pi / 7; // -60 derece dönüş
          }
                  ...
                  children: [
                    ...
                    TextButton(
                      onPressed: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(builder: (context) => SevenPoint()),
                        ); // Restart the game
                      },
...
class HeptagonPainter extends CustomPainter {
  ...
  HeptagonPainter(this.rotation, this.screenWidth, this.screenHeight);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 7; i++) {
      ...
      if (i == 0) {
        heptagonPath.moveTo(center.dx + heptagonSize * math.cos(angle) / 2,
            center.dy - heptagonSize * math.sin(angle) / 2);
      } else {
        heptagonPath.lineTo(center.dx + heptagonSize * math.cos(angle) / 2,
            center.dy - heptagonSize * math.sin(angle) / 2);
      }
    }
    heptagonPath.close();

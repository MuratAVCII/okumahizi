...
class HexaGram extends StatefulWidget {
  const HexaGram({Key? key}) : super(key: key);

  @override
  _HexaGramState createState() => _HexaGramState();
}

class _HexaGramState extends State<HexaGram> {
                  ...
                  children: [
                    ...
                    TextButton(
                      onPressed: () {
                        Navigator.pushReplacement(
                          context,
                          MaterialPageRoute(builder: (context) => HexaGram()),
                        ); // Restart the game
                      },
...
class HexagramPainter extends CustomPainter {
  ...
  HexagramPainter(this.rotation, this.screenWidth, this.screenHeight);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if (i == 0) {
        trianglePath1.moveTo(
          center.dx + hexagonSize * math.cos(angle) / 2,
          center.dy - hexagonSize * math.sin(angle) / 2,
        );
      } else {
        trianglePath1.lineTo(
          center.dx + hexagonSize * math.cos(angle) / 2,
          center.dy - hexagonSize * math.sin(angle) / 2,
        );
      }
    ...
    // İkinci eşkenar üçgen (ilkine ters)
    Path trianglePath2 = Path();
    for (int i = 0; i < 3; i++) {
      double angle = (math.pi / 2 + math.pi / 3 + i * 2 * math.pi / 3);
      if (i == 0) {
    ...
    final Offset redDotPosition = Offset(
      center.dx + hexagonSize * math.cos(math.pi / 2) / 2,
      center.dy - hexagonSize * math.sin(math.pi / 2) / 2,
    );

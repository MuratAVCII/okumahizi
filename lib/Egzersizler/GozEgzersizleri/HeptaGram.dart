...
class HeptaGram extends StatefulWidget {
  const HeptaGram({Key? key}) : super(key: key);

  @override
  _HeptaGramState createState() => _HeptaGramState();
}

class _HeptaGramState extends State<HeptaGram> {
      ...
      if (!_paused) {
        setState(() {
          if (isPositiveRotation) {
            _rotation += 2 * math.pi / 3.5; // 60 derece dönüş
          } else {
            _rotation -= 2 * math.pi / 3.5; // -60 derece dönüş
          }
                  ...
                  children: [
                    ...
                    TextButton(
                      onPressed: () {
                        Navigator.pushReplacement(
                          context,
                          MaterialPageRoute(builder: (context) => HeptaGram()),
                        ); // Restart the game
                      },
...
class HeptagramPainter extends CustomPainter {
  final double rotation;
  final double screenWidth;
  final double screenHeight;

  HeptagramPainter(this.rotation, this.screenWidth, this.screenHeight);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    double heptagonSize = size.width;
    ...
    Path heptagramPath = Path();
    for (int i = 0; i < 7; i++) {
      double angle = (math.pi / 2 + i * 4 * math.pi / 7);
      if (i == 0) {
        heptagramPath.moveTo(
          center.dx + heptagonSize * math.cos(angle) / 2,
          center.dy - heptagonSize * math.sin(angle) / 2,
        );
      } else {
        heptagramPath.lineTo(
          center.dx + heptagonSize * math.cos(angle) / 2,
          center.dy - heptagonSize * math.sin(angle) / 2,
        );
      }
    ...
    canvas.drawPath(heptagramPath, paint);

    final double dotRadius = screenHeight / 50;
    final Offset redDotPosition = Offset(
      center.dx + heptagonSize * math.cos(math.pi / 2) / 2,
      center.dy - heptagonSize * math.sin(math.pi / 2) / 2,
    );

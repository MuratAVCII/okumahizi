...
void main() {
  runApp(const MaterialApp(home: VurgulananKelimeler()));
}

class VurgulananKelimeler extends StatefulWidget {
  const VurgulananKelimeler({super.key});

  @override
  _VurgulananKelimelerState createState() => _VurgulananKelimelerState();
}

class _VurgulananKelimelerState extends State<VurgulananKelimeler> {
            ...
            TextButton(
              onPressed: () {
                ...
                Navigator.pushReplacement(
                    context,
                    MaterialPageRoute(
                        builder: (context) => const VurgulananKelimeler()));
              },
    ...
    for (int i = 0; i < _currentChunk.length; i++) {
      ...
      spans.add(TextSpan(
        ...
        style: TextStyle(
          ...
          color: globalIndex >= _currentWordIndex - _wordsToHighlight &&
                  globalIndex < _currentWordIndex
              ? Colors.black
              : Colors.black.withOpacity(
                  0.3), // Kelimeleri kırmızıya döndür, aksi halde gri
        ),
      ));
    }
